...
/// </summary>
public class BonusPousseeCreator : Bonus
{
    override
    public bool OnConsumption() //Gestion de la consommation de l'item
    {
        if (!consumed)
        {
            ...
            startTime = Time.time;
            Debug.Log("D�but Bonus Poussee");

            //Effet du bonus 
            GameObject.Find("Player").GetComponent<PlayerBonusCreator>().poussee = true;
    ...
    public bool CheckEnd()
    {
        if (Time.time >= startTime + duration && !end) //Supprimer le bonus de la liste ou assez longue ? Probl�me avec le destroy ?
        {
            ...
            GameObject.Find("Player").GetComponent<PlayerBonusCreator>().poussee = false;
