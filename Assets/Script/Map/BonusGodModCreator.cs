...
/// </summary>
public class BonusGodModCreator : Bonus
{
    ...
    public bool OnConsumption()
    {
        if (!consumed)
        {
            ...
            startTime = Time.time;
            Debug.Log("D�but Bonus God Mod");
            GameObject.Find("GameMaster").GetComponent<LifeManagerCreator>().hasGodMode = true;
    ...
    public bool CheckEnd()
    {
        if (Time.time >= startTime + duration && !end)
        {
            ...
            power -= 1;
            GameObject.Find("GameMaster").GetComponent<LifeManagerCreator>().hasGodMode = false;
            end = true;
            return true;
        }
