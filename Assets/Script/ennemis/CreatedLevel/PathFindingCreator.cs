...
public class PathFindingCreator : MonoBehaviour
{
	...
	public Vector2 FindFurthestPoint(int vision)
	{
		...
		furthest = furthestGroup[GameObject.Find("GameMaster").GetComponent<MapReader>().random.Next(0, furthestGroup.Count)];
	...
	void RetracePath(Node a, Node endNode)
	{
		...
		GetComponent<EnnemisCreator>().path = path;
	}
