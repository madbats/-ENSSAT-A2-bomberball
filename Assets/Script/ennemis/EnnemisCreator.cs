...
public abstract class EnnemisCreator : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Vector2.Distance(this.transform.position, gameMaster.GetComponent<GameMasterCreator>().playerObject.transform.position) < 1f)
        {
            gameMaster.GetComponent<LifeManagerCreator>().Death();
        }
    }
